...
app.get('/teams/name/:teamName', (req, res) => {
  ...
  let q = "SELECT * FROM teams t WHERE t.name like '%"+teamName+"%'";
...
app.post('/wishlist/:userId', (req, res) => {
  ...
  let insertQuery = 'INSERT INTO wishlist (user_id,jersey_id) VALUES('+ userId+','+jerseyId+')';
...
app.post('/collection/:userId', (req, res) => {
  const userId = req.params.userId;
  const jerseyId = req.body.jerseyId;
  let insertQuery = 'INSERT INTO collection (user_id,jersey_id) VALUES('+ userId+','+jerseyId+')';
...
app.get('/wishlist/:userId', (req, res) => {
  ...
  // Here, you can write your logic to retrieve data for the specified team ID from your database or any other data source
  pool.query('SELECT * FROM jerseys j, teams t, users u, wishlist c WHERE c.user_id=u.id and t.teamId=j.teamId and j.jerseyID=c.jersey_id and c.user_id=?', userId, (err, results) => {
    if (err) {
      console.error('Error fetching jerseys from the database:', err);
      res.status(500).json({ error: 'Internal server error' });
    } else {
